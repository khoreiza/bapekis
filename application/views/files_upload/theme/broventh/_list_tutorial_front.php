...
<?php foreach($publications as $pub){?>
	<div style="margin-top: 25px;">
		...
		<?php }?>
			<span style="height: 22px; width: 22px; font-size: 12px; padding: 3px 0 0 1px;"></span>
		</div>
		<div class="broventh_card" style="padding: 10px 15px 10px 15px; margin-top: -32px; <?=($pub != end($publications)) ? "border-bottom:1px solid #e2e2e2" : ""; ?>">
			<?php if(is_array($submodul)){?>
				<center>
					<a onclick="see_all_files('<?= $modul?>','<?= $pub['submodul']?>','<?=(isset($ownership_id)) ? $ownership_id : ""?>');"><img src="<?=base_url()?>assets/img/owl/education.png" style="height: 35px;">
