...
class Mfiles_upload extends CI_Model {
    ...
    function insert_by_param($title, $file_name, $ext, $description, $user_id, $modul, 
        ...
        $file_atch['modul'] = $modul;
        $file_atch['sub_modul'] = $submodul;
        $file_atch['user_allowed'] = $user_allowed;
        $file_atch['type'] = $type;
        $file_atch['cust_type'] = $cust_type;
        $file_atch['created'] = date("Y-m-d h:i:s");
        $file_atch['ownership_id'] = $ownership_id;
    ...
    function get_publication_files($modul,$submodul,$count_file){
        if(is_array($submodul)){
            foreach($submodul as $sub){
                $publications[$sub] = Array('submodul' => $sub, 'content' => $this->get_all_files_upload_modul_submodul($modul,$sub,$count_file));
            }
        }else{
            $publications[$submodul] = Array('submodul' => $submodul, 'content' => $this->get_all_files_upload_modul_submodul($modul,$submodul,$count_file));
        }
        return $publications;
    }

    function get_publication_files_where($modul,$submodul,$count_file, $arr_where){

        if(is_array($submodul)){
            foreach($submodul as $sub){
                if(isset($arr_where['ownership_id']) && $arr_where['ownership_id']){
                    $this->db->where('ownership_id',$arr_where['ownership_id']);
                }
                $publications[$sub] = Array('submodul' => $sub, 'content' => $this->get_all_files_upload_modul_submodul($modul,$sub,$count_file));
            }
        }else{
    ...
    function update_db($program,$id,$db){
        $this->db->where('id',$id);
        if($this->db->update($db, $program)){
            return $id;
        }
    }
	
	function update_db_all($program,$db){
    ...
    function delete_with_files_ownership_with_folder($ownership_id,$modul,$sub_modul,$delete_folder){
        $files=$this->get_files_upload_by_ownership_id($modul, $sub_modul, $ownership_id);
        $folder="";
        foreach($files as $file){
            if($file->full_url){
        ...
        if($delete_folder){
            if(file_exists($folder."thumb/") && count(glob($folder."thumb/"."*")) === 0) rmdir($folder."thumb/");
            if(file_exists($folder) && count(glob($folder."*")) === 0)rmdir($folder);
        }
        return true;
    }
    ...
    function get_latest_date_column($db,$column){
        $this->db->select('MAX('.$column.') as latest_date');
        $realizations = $this->db->get($db);
    ...
    function get_form_element($array){
        ...
        foreach ($array as $row) {
            if($this->input->post($row) || $this->input->post($row) == 0) $result[$row] = $this->input->post($row);
        }
        return $result;
    }


    function get_form_element_format($array,$type,$data){
        if($type == "date"){
            foreach ($array as $row) {
                if(isset($data[$row]) && $data[$row]){$data[$row] = DateTime::createFromFormat('d M Y', $data[$row])->format('Y-m-d');}
            }   
        }elseif($type == "number"){
            foreach ($array as $row) {
                if(isset($data[$row]) && $data[$row]) $data[$row] = str_replace(",", "", $data[$row]);
            }
        }elseif($type == "upper"){
            foreach ($array as $row) {
                if(isset($data[$row]) && $data[$row]) $data[$row] = strtoupper($data[$row]);
            }
        }
        return $data;
    ...
    function search_db_content_how_query_match($db, $arr_col, $search, $arr_where, $limit,$order_by,$how,$how_match){
        if($arr_where){
            $this->db->where($arr_where);
        }
        if($arr_col){
            ...
            if($how_match == "left") $search_value = "'%".$search."'";
            elseif($how_match == "right") $search_value = "'".$search."%'";
            elseif($how_match == "both") $search_value = "'%".$search."'";
    ...
    function upload_files($arr_data,$path,$modul,$submodul,$ownership_id,$to_files_upload,$make_thumbnail){
        //$this->upload->reset_multi_upload_data();
        
        if(is_array($arr_data)){
            $form = $arr_data['form'];

            if(isset($arr_data['file_name'])) $file_name = $arr_data['file_name'];
        }
        else{
            $form = $arr_data;
        }
    ...
    function upload_file($arr_data,$path,$modul,$submodul,$ownership_id,$to_files_upload,$make_thumbnail){
    ...
    function exceldatetoNormalDate($excel_date){
        $InvDate = PHPExcel_Shared_Date::ExcelToPHPObject($excel_date);
        return $InvDate->format('Y-m-d');
    }
    ...
    function read_excel_with_sheet_name($file, $sheet_name){
        $arrres = array();
    ...
    function get_pic_all(){
        $this->db->select("*")->from("user")->order_by("full_name asc");
        $q = $this->db->get();
        return $q->result_array();
    }

    function print_template($filename_template,$modul,$submodul,$ownership_id,$array_value){
        $printed_date = date("dMY his");
        $template_path = "assets/template";
        $template_file = $template_path.'/'.$filename_template.".docx";
        $print_path = "assets/print/".$modul;
        $print_file = $print_path."/".$filename_template."_".$ownership_id."_".$printed_date.".docx";
        if (!is_dir($template_path)) {
            mkdir($template_path, 0777, true);
        }
        if (!is_dir($print_path)) {
            mkdir($print_path, 0777, true);
        }
        $this->load->library('word');
        // directory temporary
        $dirname = uniqid('', true);
        mkdir('assets/temp/'.$dirname,0777,true);
        \PhpOffice\PhpWord\Settings::setTempDir('assets/temp/'.$dirname);
        
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor($template_file);
        if(is_array($array_value)){
            foreach($array_value as $k=>$v){
                if(is_array($v)){
                    $templateProcessor->cloneRow($k, $v['number']);
                    foreach($v as $k1=>$v1){
                        $templateProcessor->setValue($k1, $v1);    
                    }
                }else{
        ...
        $templateProcessor->setValue('printed_date', date('d-M-Y H:i:s A',strtotime($printed_date)));
        $templateProcessor->saveAs($print_file);
        return $print_file;
    }
}
