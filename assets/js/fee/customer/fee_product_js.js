function get_realization_product_by_cbgroup(v_cbgroup, chart_div){
	var result=[];
	$.getJSON(
		...
		function(data,status,xhr){
			if(status=="success"){
				draw_pie_realization_product(data,chart_div);
...
function draw_table_realization_product(v_data, v_id_div){
	...
	$.each( v_data.realizations, function( key, val ) {
		...
		"<td style=\"text-align:center\">"+parseFloat(val.percentage).toFixed(2)+"%</td></tr>");
	});
	$(v_id_div).html(items);
}

function update_highlight(v_data){
	var highest_product = "";
	if(v_data.realizations!=null){
		if(v_data.realizations[0]!=null){
			highest_product=v_data.realizations[0].fee_group;
			highest_pct = v_data.realizations[0].realization/v_data.total_amount*100;
		}
	}
	var tot_realz_fee = get_show_number(v_data.total_amount).toFixed(get_bhd_comma_val(get_show_number(v_data.total_amount)))+" "+get_show_number_satuan(v_data.total_amount);
...
function load_fee_product_summary(gv_cbgroup, gv_product, chart_div){
	...
	get_realization_product_by_cbgroup(gv_cbgroup, chart_div);
	get_realization_product_by_product(gv_cbgroup);
}

function load_fee_summary(gv_cbgroup, gv_product, chart_div){
	load_fee_product_summary(gv_cbgroup, gv_product, "chartdiv_composition");
}
