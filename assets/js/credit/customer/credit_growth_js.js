function get_credit_growth(v_report_type, v_cbgroup, v_product, v_valuta, v_sort, v_limit, v_id_div){
	...
	*/
	$.getJSON(
		config.base+"api_credit/query_growth_summary",
		{
		...
		function(data, status, xhr){
			...
			if(status=="success"){
				$.each( data.results, function( key, val ) {
					...
					items.push("<tr>"+
	    			"<td style='padding-left:5px;'>"+val.customer_name+"</td>"+
...
function get_credit_summary(v_report_type, v_command_date, v_cbgroup, v_product, v_valuta, v_id_div){
	/**
	*	input 
	...
	*/
	...
	//var result=[];
	$.getJSON(
		config.base+"api_credit/query_credit_summary",
		{
	...
	// $.ajax({
	//     type: 'GET',
	//     url: config.base+"api_credit/query_credit_summary",
	//     dataType: 'json',
...
function draw_YoY_graph(v_data, v_id_div){
	...
	$.each( v_data.results, function( key, val ) {
		...
		var newDate = date.toString('MMM yyyy');
	 	var amount = 0;
	 	if(val.amount!=0){amount = get_show_number(val.amount).toFixed(get_bhd_comma_val(get_show_number(val.amount)));}
	 	data_provider.push(
		    {
			    date: newDate, 
			    amount: val.amount,
...
function draw_MtD_graph(v_data, v_id_div){
	...
	$.each( v_data.results, function( key, val ) {
		var date = new Date(val.date); var page_type = "chartdiv_YtD_growth";
		if(v_id_div == page_type){
	...
	var chart = AmCharts.makeChart(v_id_div, {
	  ...
	  graphs: [
	    ...
	    bullet: "round",
	    labelText: "[[amount_show]] [[satuan]]",
	    bulletBorderColor: credit_color,
...
function load_credit_growth_summary(gv_cbgroup, gv_product, gv_valuta, growth_type, chart_div){
		...
		//for(i = 0; i < growth_arr.length; ++i){
			...
			get_credit_growth(growth_type, gv_cbgroup, gv_product, gv_valuta, "desc", 10, "#growth_plus_"+growth_type);
			// Year on Year - Top 10 Customer Group with the lowest growth
			get_credit_growth(growth_type, gv_cbgroup, gv_product, gv_valuta, "asc", 10, "#growth_min_"+growth_type);
			
			// /*** Initial Load Fund Summary ***/
			get_credit_summary(growth_type,"IN", gv_cbgroup, gv_product, gv_valuta, chart_div);
