...
function draw_customer_realization_graph(v_cbgroup, v_product, v_valuta, v_id_div, v_id_div_highlights,v_id_div_pie){
	...
	*/
	...
	$.getJSON(
		config.base+"api_credit/get_top_customer_realization",
		{
			cbgroup: v_cbgroup,
			product: v_product,
			valuta: v_valuta,
...
function draw_pareto_pie(v_id_div){
	
}


function load_credit_realization_summary(gv_cbgroup, gv_product, gv_valuta, div_graph){
...
function load_credit_summary(gv_cbgroup, gv_product, gv_valuta){
	load_credit_realization_summary(gv_cbgroup, gv_product, gv_valuta, "chartdiv_pareto");
}
