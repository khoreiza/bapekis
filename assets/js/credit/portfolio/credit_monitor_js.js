function get_current_jammed_loan(v_cbgroup, v_product, v_valuta){
    $.getJSON(config.base+"api_credit/get_current_jammed_loan", {
        ...
        if (status == "success") {
            ...
            $('#current_jammed_loan_table').append("<tr style=\"font-weight:bold;\">" +
                    ...
                    "<td> 100% </td></tr>");
			$("#monitoring_highlight").html(addCommas((tot_npl*1).toFixed(2))+"%");
...
function get_customer_loan_ending(v_cbgroup, v_product, v_valuta) {
    $.getJSON(config.base+"api_credit/get_customer_loan_ending", {
        ...
        if (status == "success") {
            ...
            if (data.result.length === 0) {
            	...
            	$('#sum_jth_tmp').html(data.result.length);
                $.each(data.result , function(key, val) {
...
function load_credit_monitoring_summary(gv_cbgroup, gv_product, gv_valuta){
    get_current_jammed_loan(gv_cbgroup, gv_product, gv_valuta);
    get_customer_npl_loan(gv_cbgroup, gv_product, gv_valuta);
    get_customer_loan_ending(gv_cbgroup, gv_product, gv_valuta);
