function get_fund_growth(v_report_type, v_cbgroup, v_product, v_valuta, v_sort, v_limit, v_id_div){
   	/**
	*	input 
	...
	*/
	$.getJSON(
		config.base+"api_fund/query_growth_summary",
		{
...
function get_fund_summary(v_report_type, v_command_date, v_cbgroup, v_product, v_valuta, v_id_div){
	/**
	*	input 
	...
	*/
	...
	//var result=[];
	$.getJSON(
		config.base+"api_fund/query_fund_summary",
		{
	...
	// $.ajax({
	//     type: 'GET',
	//     url: config.base+"api_fund/query_fund_summary",
	//     dataType: 'json',
...
function draw_MtD_graph(v_data, v_id_div){
	...
	$.each( v_data.results, function( key, val ) {
		var date = new Date(val.date); var page_type = "chartdiv_MtD_growth";
...
function load_fund_growth_summary(gv_cbgroup, gv_product, gv_valuta, growth_type, chart_div){
		...
		//for(i = 0; i < growth_arr.length; ++i){
			if(page_type == chart_div){
				if(cust_type != "customer"){
					...
					get_fund_growth(growth_type, gv_cbgroup, gv_product, gv_valuta, "desc", 10, "#growth_plus_"+growth_type);
					// Year on Year - Top 10 Customer Group with the lowest growth
					get_fund_growth(growth_type, gv_cbgroup, gv_product, gv_valuta, "asc", 10, "#growth_min_"+growth_type);
				}
				get_highlight_summary(growth_type,"IN", gv_cbgroup, gv_product, gv_valuta, "."+growth_type+"_highlight");
			}
			// /*** Initial Load Fund Summary ***/
			get_fund_summary(growth_type,"IN", gv_cbgroup, gv_product, gv_valuta, chart_div);
		//}
