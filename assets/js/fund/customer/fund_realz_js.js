...
function draw_customer_realization_graph(v_cbgroup, v_product, v_valuta, v_id_div, v_id_div_highlights,v_id_div_pie){
	...
	*/
	...
	$.getJSON(
		config.base+"api_fund/get_top_customer_realization",
		{
			cbgroup: v_cbgroup,
			product: v_product,
			valuta: v_valuta,
...
function load_fund_realization_summary(gv_cbgroup, gv_product, gv_valuta, div_graph){
...
function load_fund_summary(gv_cbgroup, gv_product, gv_valuta){
	load_fund_realization_summary(gv_cbgroup, gv_product, gv_valuta, "chartdiv_pareto");
}
