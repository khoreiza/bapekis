function change_element_filter(gv_cbgroup, gv_product, gv_valuta){
	if(gv_cbgroup=="all"){el_grp = "";}else{el_grp = gv_cbgroup;}
	...
	else if(gv_product == "DEPO"){el_prd = "Deposito";}else{el_prd = "TabBis";}
		...
		function(data, status, xhr){
			...
			if(status=="success"){
				$.each( data.results, function( key, val ) {
					...
					items.push("<tr>"+
	    			"<td style='padding-left:5px;'>"+val.customer_group+"</td>"+
...
function get_fund_summary(v_report_type, v_date, v_command_date, v_cbgroup, v_product, v_valuta, v_id_div){
	/**
	*	input 
...
function draw_YoY_graph(v_date, v_command_date, v_cbgroup_id, v_product, v_valuta, v_id_div){
	...
	*/
	...
	// OPEN
	var data_json = get_fund_summary(v_date, v_command_date, v_cbgroup_id, v_product, v_valuta);
...
function get_highlight_summary(v_date, v_command_date, v_cbgroup, v_product, v_valuta, v_id_div){
	...
	*/
	...
	var result=[];
	$.getJSON(
		"http://localhost/cbic/api_fund/query_fund_summary",
		{
			date: v_date,
			command_date: v_command_date,
			cbgroup: v_cbgroup,
